...
//Xử lý phân trang
$all_portfolio_categories = getRows("SELECT id FROM `portfolio_categories` $filter");
// 1. Xác định số lượng bản ghi 1 trang
$perPage = _PER_PAGE;

// 2. Tính số trang
$maxPage = ceil($all_portfolio_categories / $perPage);
...
$listPortolioCategories = getRaw("SELECT `portfolio_categories`.*, `users`.fullname, `users`.id as user_id, 
(SELECT COUNT(`portfolios`.id) FROM `portfolios` WHERE `portfolio_categories`.id = `portfolios`.portfolio_categories_id) as portfolioCount
 FROM `portfolio_categories` INNER JOIN `users` ON `portfolio_categories`.user_id = `users`.id $filter ORDER BY `create_at` DESC LIMIT $offset, $perPage");
// echo "<pre>";
// print_r($listPortolioCategories);
// echo "</pre>";
      ...
      <div class="col-lg-8">
        <div class="card ">
          <div class="card-header">
            <h3 class="card-title font-weight-light">Danh sách danh mục dự án</h3>
          </div>
          <form role="form">
            <div class="card-body">
              <table class="table table-striped table-bordered">
                ...
                <tbody>
                  <?php
                  $temp = $offset;
                  if (!empty($listPortolioCategories)) :
                    foreach ($listPortolioCategories as $item) : ?>
                      <tr>
                        ...
                        <td colspan="2" class="text-center align-middle">
                          <a href="<?php echo getLinkAdmin('portfolio_categories', '', ['id' => $item['id'], 'view' => 'edit']) ?>" class="btn btn-sm btn-warning mr-2">
                            <i class="fa fa-edit"></i> Sửa</a>
                          <a onclick="return confirm('Bạn chắc chắn xoá dữ liệu này!')" href="<?php echo getLinkAdmin('portfolio_categories', 'delete', ['id' => $item['id']]) ?>" class="btn btn-sm btn-danger">
          ...
          <div class="card-footer">
            <?php echo getPaging($page, 'portfolio_categories', $queryString, $maxPage); ?>
          </div>
        </div>
      </div>
    </div>
